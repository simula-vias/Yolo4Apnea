        ...
        with self.app.test_client() as c:
            ...
            while i < 3000:
                ...
                if i+duration < sliding_window_duration:
                    ...
                    self.assertEqual(json_data["start_index"]+sliding_window_duration,i+duration)

                print(f"i: {i} duration: {duration} signal length:{len(signal)} pred length: {len(json_data['last_predictions'])}")
                if i + len(signal) < sliding_window_duration:
                    self.assertEqual(len(json_data["last_predictions"]),i + len(signal))
                else:
                    self.assertEqual(len(json_data["last_predictions"]),sliding_window_duration)
